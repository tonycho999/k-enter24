...
def extract_trending_entities(news_items, recent_titles):
    ...
    recent_info = f"\n[주의: 최근 작성한 기사 제목들]\n{recent_titles}\n위 기사에 나온 영화나 배우는 절대 제외해." if recent_titles else ""
    
    system_prompt = f"""너는 영화 트렌드 분석가야. 뉴스 제목들에서 가장 화제가 되는 특정 '한국 영화 제목' 또는 '한국 영화배우 이름'을 화제성 순으로 '3개' 찾아줘.
    {recent_info}
    반드시 JSON 형식으로 답변해. 형식: {{"keywords": ["1위영화", "2위배우", "3위영화"]}}"""
    
    response_data = get_ai_response(system_prompt, str(titles))
    return response_data.get('keywords', ["최신 영화"])
...
def main():
    ...
    print(f"🔥 AI가 뽑은 K-MOVIE 후보군 Top 3: {trending_entities}")
    ...
    for entity in trending_entities:
        ...
        제공된 K-Movie 뉴스를 바탕으로 해외 영화 팬들이 흥미를 가질 만한 멋진 영어 제목과 깊이 있는 영어 리뷰 기사를 작성해 줘.
        [규칙]
        ...
        title = article_data.get('title', f'{entity} - Box Office Update')
    ...
    if not saved_successfully:
        print("\n❌ 3개의 영화 후보를 모두 뒤졌지만, 사진 퀄리티 등을 만족하는 이슈가 없어 포기합니다.")
        exit(1)
